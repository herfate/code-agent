...
/** 业务知识编排（类型 9）：必须且仅能配置 1 个 git 仓库 */
function refineBusinessCoreParentTaskFields(
  data: {
    task_type?: (typeof PARENT_AGENT_TYPE)[keyof typeof PARENT_AGENT_TYPE];
    description?: string;
    gitRepos: z.infer<typeof gitRepoPair>[];
  ...
  if (data.task_type !== PARENT_AGENT_TYPE.BusinessCore) return;
  if (!data.description?.trim()) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: "description is required for task_type 9",
      path: ["description"],
    });
  }
  if (data.gitRepos.length !== 1) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: "业务知识任务仅支持且必须填写 1 个代码仓库",
      path: ["gitRepos"],
    });
  }
}
...
export const zCreateParentTaskBody = z
  ...
  .superRefine((data, ctx) => {
    ...
    refineBusinessCoreParentTaskFields(data, ctx);
